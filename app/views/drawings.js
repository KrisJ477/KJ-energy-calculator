  ...
  for (const file of files) {
    ...
    if (name.endsWith('.pdf')) {
      ...
      for (let i = 0; i < doc.numPages; i++) {
        ...
        const info = await inspectPdfPage(doc, i);
  ...
  const sheetRows = p.sheets.map((s) => {
    ...
    return {
      cells: [
        ...
        select(['base', 'scale-reference', 'change-patch', 'cross-check', 'ignore'].map((r) => [r, t(`drawings.roles.${r}`)]), s.role, (v) => set((pr) => (pr.sheets.find((x) => x.id === s.id).role = v))),
