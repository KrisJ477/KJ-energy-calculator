...
Date: 2026-09-26
Status: Released for build. Section 9 lists the remaining tuning items; none of them blocks the build.
...
- Section 9 lists the remaining tuning items (starting values to be refined in use). Build with the stated starting values; do not block on them.
- Tests (the only tests to build): automated tests for the calculation engine (5), geometry processing (floor/ceiling pieces and slivers 3.4, geometry cleanup 4.5) and scale logic (4.4); plus an end-to-end run in manual mode (2) on real test drawings.
...
- AI provider layer: every AI job (reads, combined passes, re-reads, annotations, command window) goes through one internal interface with two interchangeable modes, API and manual. The rest of the app does not know which mode answered.
  - API mode (production default): requests go through the Worker to the Anthropic API.
  - Manual mode (development and testing, also available in the finished app behind a developer setting): instead of calling the API, the app writes each request as a request bundle (request id, job type, the model that would have been used, prompt text and prompt version, output schema, project brief, and the images/tiles as files). A response file with the same request id is then loaded back. It is validated against the output schema exactly like an API response; an invalid response is shown as an error and the request stays pending. Responses may arrive in any order.
  - Exchange: through a folder the user picks once (browser File System Access API, Chrome/Edge): the app writes requests there and picks up responses automatically. Fallback in other browsers: download request bundles and upload responses by hand. This lets a Claude session running the app locally (e.g. in a development environment with a browser) answer requests with no API cost.
  - Manual mode needs no Worker and no password. A clear "MANUAL MODE" badge is shown while it is active. Reads answered in manual mode record "manual" as their model (6) and the origin tag (3.10) records it too.
...
- Slivers caused by imperfect alignment are absorbed into the neighbouring piece they share the longest edge with. A sliver is a piece whose narrowest width is under 150 mm (whatever its length), or any piece under 0.1 m². Width is the criterion because floors can still differ by ~1 % after scale correction, giving strips up to ~10 cm wide but building-long. Absorbed slivers are counted per floor ("Floor 3: 7 slivers absorbed") and can be highlighted. Threshold on the settings page (3.14).
- Area with no room above: goes to roof or outside air. Area with no room below: goes to the ground slab (3.5) or outside air, or to a space outside the project where the user marks it so (3.3; e.g. a garage below that is not part of the project).
...
- sliver threshold for floor/ceiling pieces (3.4): 150 mm narrowest width, 0.1 m² minimum area
...
- AI settings: model per AI job (6), target resolution (8 mm/px) and tile overlap (2 m) (4.2), manual mode on/off (2, developer setting)
...
- Per sheet: an overview call and detail tile calls (tiling below), run in parallel. The stitched composite of a multi-sheet floor is never sent as one image.
- Then one combined pass per floor over the extracted data of all its tiles and sheets, plus the extracted data of the vertical drawings (and crops where needed), for cross-drawing work: merging tiles and sheet seams (4.3), matching openings between plans and vertical drawings, comparing the same wall in neighbouring apartments and on the floor above, and gap classification.
...
Vertical drawings:
- Scaled like plans, from a known measurement (written heights or levels, or a scale bar), then checked against one reference feature shared with the plans (one of the two reference walls). Only one vector is available because of the viewing angle, so no two-wall cross-check.
...
- On request, the app sends the region image plus comments to the regional re-read model (default Haiku, 6), which re-reads that region only and updates it. Changes are highlighted and recorded on the object (3.10).
...
- Coverage: parts of a floor that no drawing covers (4.2, 3.13).
...
- Opus: per-sheet reads (overview and detail tiles), the combined pass per floor, and any full-floor re-read.
...
- Cost estimate (rough, list prices at the time of writing: Opus 5 $5 / $25 per million input / output tokens, Opus 5.5 $4 / $20, Sonnet 5 $2 / $10, Haiku 4.5 $1 / $5; check current prices at build time):
  ...
  - Levers if needed: Opus 5.5 (−20 %), Sonnet 5 (−60 %, quality to be tested), lower effort, reading only the floors that differ when floors are copied (not current behaviour: 4.4 reads every floor and diffs; changing it is a spec decision), and prompt caching of the fixed instructions. Measure real token use on a few tiles before relying on these numbers.
...
- Whether an AI result came from the API or from manual mode (2).
...
## 9. Tuning items (starting values; none blocks the build)
